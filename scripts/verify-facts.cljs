...
;;
;; THE TWO WAYS A HOST SAYS NOTHING, BOTH MEASURED 2026-09-01.
;;
;; www.imf.org and www.census.gov are behind bot walls that answer 403 to
;; an automated client, and stat.unido.org serves a Cloudflare challenge --
;; none of them is cited here. The challenge detector (below, on the AWS WAF
;; signature) stays anyway, because a host that starts interposing a
;; challenge mid-run must read as REFUSED -- exit 2 -- and not as a register
;; error.
;;
;; comtradeplus.un.org is the subtler case: an Angular SPA that answers
;; **200** with the same 3228-byte shell at every path, real or invented.
;; Status discriminates nothing there, so the host entity declares
;; :serves-spa, pins the shell title in :host/spa-title, and the title is
;; the only guard. The three hosts that DO refuse (www.customs.go.jp,
;; ec.europa.eu, unstats.un.org) all answer **404**, not 403, for a path
;; that does not exist -- each with its own body and title. The status each
;; host actually returned is pinned in :host/missing-status and compared on
;; every run; a checker hardwired to 403, or one reading any 4xx as
;; "blocked, cannot measure", would misreport hosts that answered clearly.
;;
  ...
  (let [host (:host/name h)
    ...
    (p/let [r (fetch-page url)]
      (cond
        ...
        ;; Measured 2026-09-01 on the marketer register: comtradeplus.un.org
        ;; is an Angular SPA that serves the SAME 3228-byte shell (title
        ;; "UN Comtrade") at every path, real or invented. Status cannot
        ;; discriminate here, and pretending otherwise would be the
        ;; host-soft-404 failure below pointed at a host that did nothing
        ;; wrong. The behaviour is declared, and what CAN be checked is: a
        ;; 2xx at the requested URL, and the shell title still the one
        ;; measured. If the host ever starts answering 404s properly, or the
        ;; shell title changes, this fails loudly.
        (= :serves-spa behaviour)
        (cond
          (not (<= 200 (:status r) 299))
          {:verdict :fail :reason :host-behaviour-drift
           :why (str host " answered HTTP " (:status r) " for "
                     NONEXISTENT-PATH ", but the register measures it as a SPA "
                     "that serves a 2xx shell at every path -- the host's "
                     "behaviour changed and the page checks resting on it no "
                     "longer rest on the measurement.")}
          (and (:host/spa-title h) (not= (:title r) (:host/spa-title h)))
          {:verdict :fail :reason :host-spa-title-drift
           :why (str host " shell title is now " (pr-str (:title r))
                     ", register measured " (pr-str (:host/spa-title h)))}
          :else
          {:verdict :pass
           :detail (str "HTTP " (:status r) " with shell title "
                        (pr-str (:title r)) " for a path that does not exist, "
                        "as declared (status cannot discriminate on this host; "
                        "the title is the guard)")})
