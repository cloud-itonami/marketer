...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "marketer-mutation-" suffix ".edn")))
...
(def with-pages
  ...
  branches."
  (subset #{"host.comtradeplus-un-org" "host.unstats-un-org"
            "law.stats-act" "page.comtradeplus-portal" "page.unstats-m49"
            "page.eurostat-nace"}))
...
(def structural
  [{:id "coverage-entry-count"
   ...
   {:id "law-entry-without-law-id"
    ...
    :mutate #(alter-entity % "law.stats-act" (fn [e] (dissoc e :egov/law-id)))
    ...
    :mutate (fn [data]
              (mapv (fn [e]
                      (if (and (= :page-identity (:source/verify e))
                               (not= "page.comtradeplus-portal" (:source/id e)))
  ...
  [{:id "nonexistent-law" :on :laws-only
   ...
   {:id "fail-outranks-refusal" :on :laws-only
    ...
    :mutate (fn [d]
              (recount
                (-> d
                    (alter-entity "law.stats-act"
                                  (fn [e] (assoc e :egov/law-title "統計報告調整法")))
                    (into UNROUTABLE-HOST))))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "page-title-drift"
    :why "a real page recorded under a title it does not carry"
    :mutate #(alter-entity % "page.unstats-m49" (fn [e] (assoc e :page/title "UNSD Methodology")))
    :want-exit 1 :want-reason "title-drift"}

   {:id "missing-text"
    :why "a string the page does not contain"
    :mutate #(alter-entity % "page.eurostat-nace"
   ...
   {:id "charset-drift"
    ...
    :mutate #(alter-entity % "page.unstats-m49" (fn [e] (assoc e :page/charset "shift_jis")))
    :want-exit 1 :want-reason "charset-drift"}

   {:id "unexpected-redirect"
    :why "a URL that lands somewhere other than where it asked"
    :mutate #(alter-entity % "page.comtradeplus-portal"
                           (fn [e] (assoc e :source/url "http://comtradeplus.un.org/")))
    :want-exit 1 :want-reason "unexpected-redirect"}

   {:id "not-2xx"
    :why "a cited page that is not there"
    :mutate #(alter-entity % "page.customs-trade-stats"
                           (fn [e] (assoc e :source/url "https://www.customs.go.jp/toukei/info/no-such-zzz.html")))
   ...
   {:id "host-status-drift"
    ...
    :mutate #(alter-entity % "host.unstats-un-org"
                           (fn [e] (assoc e :host/missing-status 500)))
